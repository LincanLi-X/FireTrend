...
def unpack_batch(batch, device: torch.device):
    if isinstance(batch, dict):
        ...
        if "future_drivers" in batch:
            raise ValueError(
                "Legacy 'future_drivers' are not accepted because they may contain "
                "target-day ERA5 observations. Use 'forecast_drivers' only for "
                "meteorological forecasts available at prediction time."
            )
        forecast_drivers = batch.get("forecast_drivers")
        forecast_drivers = (
            forecast_drivers.to(device, non_blocking=True)
        ...
        valid_region_mask = batch.get("valid_region_mask")
        valid_region_mask = (
            valid_region_mask.to(device, non_blocking=True)
        ...
        target_index = None
        valid_region_mask = batch[4].to(device, non_blocking=True) if len(batch) >= 5 else None
    else:
    ...
    X_meteo = X[:, :, 1:9]
    X_geo = X[:, :, 9:19]
    return X_fire, X_meteo, X_geo, drivers, forecast_drivers, y_class, target_index, valid_region_mask
...
def run_epoch(model, dataloader, optimizer, loss_fn, device, stage, logger, region, epoch, forecast_horizon):
    ...
    for batch in progress:
        X_fire, X_meteo, X_geo, drivers, forecast_drivers, y_class, _, valid_region_mask = unpack_batch(batch, device)
        ...
        outputs = model(
            ...
            X_geo=X_geo,
            X_drivers=drivers,
            X_forecast_drivers=forecast_drivers,
...
def evaluate(model, dataloader, device, logger, region, forecast_horizon=1, results_dir=None):
    model.eval()
    logits_all, targets_all, probs_all, target_indices_all, masks_all = [], [], [], [], []
    wind_u_all, wind_v_all = [], []
    for batch in tqdm(dataloader, desc=f"[{region}] evaluation"):
        X_fire, X_meteo, X_geo, drivers, forecast_drivers, y_class, target_index, valid_region_mask = unpack_batch(batch, device)
        outputs = model(
            X_fire=X_fire,
            X_meteo=X_meteo,
            X_geo=X_geo,
        ...
        if valid_region_mask is not None:
            masks_all.append(valid_region_mask.detach().cpu())
        if target_index is not None:
            target_indices_all.append(target_index.detach().cpu())
        driver_ref = (
            forecast_drivers[:, forecast_horizon - 1]
            if forecast_drivers is not None and forecast_drivers.size(1) > 0
            else drivers[:, -1]
        )
    ...
    valid_region_masks = torch.cat(masks_all, dim=0) if masks_all else None
    ignore_index = int(getattr(dataloader.dataset, "ignore_index", -100))
    ...
    metrics["PDS"] = compute_pds(
        probabilities,
        wind_u,
        wind_v,
        class_index=min(2, logits.size(1) - 1),
        valid_region_mask=valid_region_masks,
    )
    metrics.update(
        compute_temporal_drift_sequence(
            probabilities,
            targets,
...
def build_dataloader(config, region, batch_size, split, shuffle, config_dir):
    ...
    return create_dataloader(
        ...
        ignore_index=int(data_cfg.get("ignore_index", -100)),
        return_dict=True,
    )
...
def build_model(config, height, width, device):
    ...
    model = FireTrendModel(
        ...
        causal_temporal_attention=bool(model_cfg.get("causal_temporal_attention", True)),
...
def main():
    ...
    loss_fn = FireTrendLoss(
        ...
        ignore_index=int(config["data"].get("ignore_index", -100)),
    ).to(device)
