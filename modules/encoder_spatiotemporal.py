"""
FireTrend Stage 1 — Two-Scale Multimodal Spatial-Temporal Encoder (Corrected)
Integrates wildfire history, meteorological, and dynamic geospatial inputs.
"""

import torch
import torch.nn as nn
import torch.nn.functional as F
from einops import rearrange
from modules.layers.temporal_transformer import TemporalTransformerLayer
from modules.layers.spatial_transformer import SpatialTransformerLayer


# -----------------------------------------------------------
# Shape Checking Utility
# -----------------------------------------------------------
def check_tensor_shape(tensor: torch.Tensor, expected_dims: int, context: str = ""):
    if tensor.ndim != expected_dims:
        raise ValueError(
            f"\033[91m[ShapeError in {context}] Expected {expected_dims} dims, "
            f"but got {tensor.ndim}. Tensor shape = {list(tensor.shape)}\033[0m"
        )
    return True


def _coerce_valid_region_mask(valid_region_mask, reference: torch.Tensor):
    """Return a boolean [B,H,W] mask aligned with a [B,T,C,H,W] tensor."""
    if valid_region_mask is None:
        return None
    B, _, _, H, W = reference.shape
    mask = torch.as_tensor(valid_region_mask, device=reference.device, dtype=torch.bool)
    if mask.ndim == 2:
        mask = mask.unsqueeze(0).expand(B, -1, -1)
    if tuple(mask.shape) != (B, H, W):
        raise ValueError(f"valid_region_mask must have shape {(B, H, W)}, got {tuple(mask.shape)}")
    if not bool(mask.any(dim=(1, 2)).all()):
        raise ValueError("Every sample must contain at least one valid spatial cell.")
    return mask


class ChannelLayerNorm3d(nn.Module):
    """Layer-normalize channels independently at every (t, h, w) position.

    Unlike BatchNorm3d, this operation does not aggregate statistics across
    the temporal axis, so a latent state at time t cannot depend on inputs from
    future time steps through normalization statistics.
    """

    def __init__(self, channels: int):
        super().__init__()
        self.norm = nn.LayerNorm(int(channels))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        check_tensor_shape(x, 5, "ChannelLayerNorm3d")
        x = rearrange(x, "b c t h w -> b t h w c")
        x = self.norm(x)
        return rearrange(x, "b t h w c -> b c t h w")


# -----------------------------------------------------------
# Multimodal Fusion Block
# -----------------------------------------------------------
class MultimodalFusion(nn.Module):
    """
    Projects score, meteorology, remote/vegetation/fuel, and static geospatial
    inputs separately, then concatenates them as in the manuscript.
    """

    def __init__(self, in_dims, embed_dim):
        """
        Args:
            in_dims (dict): {'fire': C_f, 'meteo': C_m, 'geo': C_g}
        """
        super().__init__()
        geo_dim = int(in_dims["geo"])
        if geo_dim >= 10:
            self.remote_indices = [0, 1, 2, 6, 7, 8, 9]
            self.geo_static_indices = [3, 4, 5]
        else:
            self.remote_indices = list(range(geo_dim))
            self.geo_static_indices = []
        remote_dim = len(self.remote_indices)
        geo_static_dim = max(1, len(self.geo_static_indices))

        self.fire_proj = nn.Conv3d(in_dims['fire'], embed_dim, kernel_size=1)
        self.meteo_proj = nn.Conv3d(in_dims['meteo'], embed_dim, kernel_size=1)
        self.remote_proj = nn.Conv3d(remote_dim, embed_dim, kernel_size=1)
        self.geo_proj = nn.Conv3d(geo_static_dim, embed_dim, kernel_size=1)
        self.concat_proj = nn.Conv3d(embed_dim * 4, embed_dim, kernel_size=1)
        self.norm = ChannelLayerNorm3d(embed_dim)

    # def forward(self, X_fire, X_meteo, X_geo):
    #     """
    #     Args:
    #         X_fire:  [B, T, C_f, H, W]
    #         X_meteo:[B, T, C_m, H, W]
    #         X_geo:  [B, T, C_g, H, W]
    #     Returns:
    #         fused: [B, T, D, H, W]
    #     """
    #     check_tensor_shape(X_fire, 5, "Fusion:X_fire")
    #     check_tensor_shape(X_meteo, 5, "Fusion:X_meteo")
    #     check_tensor_shape(X_geo, 5, "Fusion:X_geo")

    #     # Rearrange for Conv3D (C at dim=1, T at dim=2)
    #     X_fire = rearrange(X_fire, "b t c h w -> b c t h w")
    #     X_meteo = rearrange(X_meteo, "b t c h w -> b c t h w")
    #     X_geo = rearrange(X_geo, "b t c h w -> b c t h w")

    #     F_fire = self.fire_proj(X_fire)
    #     F_meteo = self.meteo_proj(X_meteo)
    #     F_geo = self.geo_proj(X_geo)

    #     # Fusion by summation (aligned across time)
    #     fused = F_fire + F_meteo + F_geo
    #     fused = self.norm(fused)
    #     fused = F.gelu(fused)

    #     # Restore to [B, T, D, H, W]
    #     fused = rearrange(fused, "b d t h w -> b t d h w")
    #     return fused


    def forward(self, X_fire, X_meteo, X_geo, return_modalities: bool = False):
        """
        Args:
            X_fire:  [B, T, C_f, H, W]
            X_meteo:[B, T, C_m, H, W]
            X_geo:  [B, T, C_g, H, W]
        Returns:
            fused: [B, T, D, H, W]
            modality_feats (optional): dict of modality tensors [B, T, D, H, W]
        """
        check_tensor_shape(X_fire, 5, "Fusion:X_fire")
        check_tensor_shape(X_meteo, 5, "Fusion:X_meteo")
        check_tensor_shape(X_geo, 5, "Fusion:X_geo")

        # --- Fire ---
        X_fire = rearrange(X_fire, "b t c h w -> b c t h w")
        F_fire = self.fire_proj(X_fire)

        # --- Meteorology ---
        X_meteo = rearrange(X_meteo, "b t c h w -> b c t h w")
        F_meteo = self.meteo_proj(X_meteo)

        # --- Remote/vegetation/fuel and static geospatial ---
        X_geo = rearrange(X_geo, "b t c h w -> b c t h w")
        F_remote = self.remote_proj(X_geo[:, self.remote_indices])
        if self.geo_static_indices:
            F_geo = self.geo_proj(X_geo[:, self.geo_static_indices])
        else:
            zeros = X_geo[:, :1] * 0.0
            F_geo = self.geo_proj(zeros)

        # --- Concatenation-based fusion ---
        fused = self.concat_proj(torch.cat([F_fire, F_meteo, F_remote, F_geo], dim=1))
        fused = self.norm(fused)
        fused = F.gelu(fused)
        fused = rearrange(fused, "b d t h w -> b t d h w")

        if not return_modalities:
            return fused

        modality_feats = {
            "fire": rearrange(F.gelu(F_fire), "b d t h w -> b t d h w"),
            "meteo": rearrange(F.gelu(F_meteo), "b d t h w -> b t d h w"),
            "remote": rearrange(F.gelu(F_remote), "b d t h w -> b t d h w"),
            "geo": rearrange(F.gelu(F_geo), "b d t h w -> b t d h w"),
        }
        return fused, modality_feats


# -----------------------------------------------------------
#  Main Encoder
# -----------------------------------------------------------
class SpatialTemporalEncoder(nn.Module):
    """
    FireTrend Multimodal Spatio-Temporal Encoder
    (Wildfire + ERA5 Meteorology + Dynamic Geo)
    """
       
    def __init__(self, in_dims, embed_dim=128, num_heads=4,
             hidden_dim=512, dropout=0.1, height=None, width=None, num_layers=1,
             causal_temporal_attention=True, verbose=False):
        super().__init__()
        self.verbose = verbose
        self.num_layers = int(max(1, num_layers))
        self.fusion = MultimodalFusion(in_dims, embed_dim)
        self.temporal_blocks = nn.ModuleList(
            [
                TemporalTransformerLayer(
                    dim=embed_dim,
                    num_heads=num_heads,
                    hidden_dim=hidden_dim,
                    dropout=dropout,
                    causal_attention=causal_temporal_attention,
                )
                for _ in range(self.num_layers)
            ]
        )
        self.spatial_blocks = nn.ModuleList(
            [
                SpatialTransformerLayer(
                    dim=embed_dim, height=height, width=width,
                    num_heads=num_heads, hidden_dim=hidden_dim, dropout=dropout
                )
                for _ in range(self.num_layers)
            ]
        )
        self.proj_out = nn.Conv3d(embed_dim, embed_dim, kernel_size=1)
        self.norm = ChannelLayerNorm3d(embed_dim)
        self.causal_temporal_attention = bool(causal_temporal_attention)


    def _project_branch(self, branch_bt, valid_region_mask=None):
        """
        Project a modality branch into the same embedding space as Z.
        Args:
            branch_bt: [B, T, D, H, W]
        Returns:
            [B, T, D, H, W]
        """
        branch_3d = rearrange(branch_bt, "b t d h w -> b d t h w")
        branch_3d = self.proj_out(branch_3d)
        branch_3d = self.norm(branch_3d)
        projected = rearrange(branch_3d, "b d t h w -> b t d h w")
        if valid_region_mask is not None:
            projected = projected * valid_region_mask[:, None, None].to(projected.dtype)
        return projected

    def forward(
        self,
        X_fire,
        X_meteo,
        X_geo,
        return_modalities: bool = False,
        valid_region_mask=None,
    ):
        """
        Args:
            X_fire:  [B, T, 1, H, W]
            X_meteo:[B, T, C_m, H, W]
            X_geo:  [B, T, C_g, H, W]
            valid_region_mask: optional boolean mask [B,H,W] or [H,W]
        Returns:
            Z: [B, T, D, H, W]
            modality_Z (optional): dict of modality tensors [B, T, D, H, W]
        """
        check_tensor_shape(X_fire, 5, "Encoder:X_fire")
        check_tensor_shape(X_meteo, 5, "Encoder:X_meteo")
        check_tensor_shape(X_geo, 5, "Encoder:X_geo")
        spatial_mask = _coerce_valid_region_mask(valid_region_mask, X_fire)
        mask_bt = spatial_mask[:, None, None] if spatial_mask is not None else None

        # Prevent background values from entering any modality projection.
        if mask_bt is not None:
            X_fire = X_fire * mask_bt.to(X_fire.dtype)
            X_meteo = X_meteo * mask_bt.to(X_meteo.dtype)
            X_geo = X_geo * mask_bt.to(X_geo.dtype)

        if self.verbose:
            print(f"[Encoder] Inputs fire={X_fire.shape}, meteo={X_meteo.shape}, geo={X_geo.shape}")

        # Step 1. Multimodal Fusion
        if return_modalities:
            fused, modality_feats = self.fusion(X_fire, X_meteo, X_geo, return_modalities=True)
        else:
            fused = self.fusion(X_fire, X_meteo, X_geo, return_modalities=False)
            modality_feats = None
        if mask_bt is not None:
            fused = fused * mask_bt.to(fused.dtype)
            if modality_feats is not None:
                modality_feats = {
                    name: feat * mask_bt.to(feat.dtype)
                    for name, feat in modality_feats.items()
                }
        if self.verbose:
            print(f"[Encoder] After Fusion: {list(fused.shape)}")

        for layer_idx, (temporal_block, spatial_block) in enumerate(zip(self.temporal_blocks, self.spatial_blocks), 1):
            fused = temporal_block(fused)
            if mask_bt is not None:
                fused = fused * mask_bt.to(fused.dtype)
            if self.verbose:
                print(f"[Encoder] Layer {layer_idx} after Temporal Transformer: {list(fused.shape)}")

            if hasattr(spatial_block, "pos_encoding"):
                h, w = fused.shape[-2:]
                pos_emb = spatial_block.pos_encoding
                pe_h, pe_w = pos_emb.pos_emb.shape[-2:]
                if pe_h != h or pe_w != w:
                    from modules.layers.spatial_transformer import SpatialPositionalEncoding
                    spatial_block.pos_encoding = SpatialPositionalEncoding(fused.shape[2], h, w).to(fused.device)
                    if self.verbose:
                        print(f"[Encoder] Reinitialized positional encoding to ({h}, {w})")

            fused = spatial_block(fused, valid_region_mask=spatial_mask)
            if mask_bt is not None:
                fused = fused * mask_bt.to(fused.dtype)
            if self.verbose:
                print(f"[Encoder] Layer {layer_idx} after Spatial Transformer: {list(fused.shape)}")

        # Step 4. Output Projection
        fused_3d = rearrange(fused, "b t d h w -> b d t h w")
        Z = self.proj_out(fused_3d)
        Z = self.norm(Z)
        Z = rearrange(Z, "b d t h w -> b t d h w")
        if mask_bt is not None:
            Z = Z * mask_bt.to(Z.dtype)

        if self.verbose:
            print(f"[Encoder] Output Z: {list(Z.shape)}")

        if not return_modalities:
            return Z

        modality_Z = {
            name: self._project_branch(feat, valid_region_mask=spatial_mask)
            for name, feat in modality_feats.items()
        }
        return Z, modality_Z


# -----------------------------------------------------------
# Unit Test
# -----------------------------------------------------------
if __name__ == "__main__":
    torch.manual_seed(0)
    B = 2      # batch size
    T = 6      # temporal sequence length (daily steps)
    H, W = 16, 16

    # --- Create dummy inputs ---
    X_fire = torch.randn(B, T, 1, H, W)        # wildfire daily
    X_meteo = torch.randn(B, T, 6, H, W)       # meteorological features (daily)
    X_geo = torch.randn(B, T, 2, H, W)         # dynamic geo features

    # --- Initialize model ---
    model = SpatialTemporalEncoder(
        in_dims={"fire": 1, "meteo": 6, "geo": 2},
        embed_dim=64,
        num_heads=4,
        hidden_dim=128,
        height=H,
        width=W,
        verbose=True
    )

    # --- Forward pass ---
    Z = model(X_fire, X_meteo, X_geo)

    print("\n Test passed!")
    print(f"Output Z shape: {list(Z.shape)}  (expected [B, T, D, H, W])")
