...
def _coerce_spatial_mask(
    valid_region_mask: torch.Tensor | None,
    reference: torch.Tensor,
) -> torch.Tensor | None:
    """Return a boolean mask [B,1,H,W] aligned with a latent map."""
    ...
    elif mask.ndim == 4 and mask.size(1) == 1:
        mask = mask[:, 0]
...
class DirectionalConv2D(nn.Module):
    ...
    def forward(
        ...
        if tuple(kernel.shape[-2:]) != (H, W):
            raise ValueError(
                f"Driver/latent spatial mismatch: kernel={tuple(kernel.shape[-2:])}, latent={(H, W)}"
            )

        target_mask = _coerce_spatial_mask(valid_region_mask, latent_map)
        if target_mask is not None:
            # Invalid latent values must never enter an effective source patch.
            latent_map = latent_map * target_mask.to(latent_map.dtype)

            # Mask both outside-state neighbors and rectangular zero-padding,
            # then renormalize the remaining weights while preserving the
            # original per-target propagation strength (normally alpha).
            neighbor_valid = F.unfold(
                target_mask.to(kernel.dtype),
                kernel_size=self.kernel_size,
                padding=self.pad_size,
            ).view(B, self.kernel_size * self.kernel_size, H, W)
            kernel_strength = kernel.sum(dim=1, keepdim=True)
            kernel = kernel * neighbor_valid
            valid_weight = kernel.sum(dim=1, keepdim=True)
            kernel = torch.where(
                valid_weight > 1e-8,
                kernel / valid_weight.clamp_min(1e-8) * kernel_strength,
        ...
        if target_mask is not None:
            propagated = propagated * target_mask.to(propagated.dtype)
        return propagated
