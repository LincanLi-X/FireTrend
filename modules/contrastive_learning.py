...
class MultiViewContrastive(nn.Module):
    ...
    def temporal_contrast(self, Z: torch.Tensor) -> torch.Tensor:
        ...
        idx = _sample_indices(cells.size(0), self.max_temporal_cells, Z.device)
    ...
    def _spatial_guided_embedding(self, Z_bt: torch.Tensor) -> torch.Tensor:
    ...
    def spatial_contrast(self, Z: torch.Tensor) -> torch.Tensor:
        """
        ...
        q_bt = self._spatial_guided_embedding(Z_bt)
        q_flat = rearrange(q_bt, "bt d h w -> bt (h w) d")
        q_flat = self._project_nd(q_flat)
        q_flat = F.normalize(q_flat, dim=-1)

        BT, N, Dp = q_flat.shape
        n_anchor = min(max(1, self.max_spatial_anchors), N)
        anchor_idx = torch.randint(0, N, (BT, n_anchor), device=Z.device)
        bt_idx = torch.arange(BT, device=Z.device)[:, None]
        anchors = q_flat[bt_idx, anchor_idx]  # [BT, A, D]
        logits = torch.einsum("bad,bnd->ban", anchors, q_flat) / self.temperature
        ...
        for dh, dw in self._neighbor_offsets():
            ...
            pos_indices.append((nh.clamp(0, H - 1) * W + nw.clamp(0, W - 1)).unsqueeze(-1))
            pos_valid.append(valid.unsqueeze(-1))
        pos_indices = torch.cat(pos_indices, dim=-1)
        pos_valid = torch.cat(pos_valid, dim=-1)

        pos_logits = torch.gather(logits, dim=2, index=pos_indices)
        pos_logits = pos_logits.masked_fill(~pos_valid, -torch.finfo(pos_logits.dtype).max)
        numerator = torch.logsumexp(pos_logits, dim=-1)
        denominator = torch.logsumexp(logits, dim=-1)
        valid_anchor = pos_valid.any(dim=-1)
        loss = -(numerator[valid_anchor] - denominator[valid_anchor]).mean()
        if self.verbose:
            print(f"[Contrast] spatial={loss.item():.4f}")
        return loss

    def cross_modal_contrast(self, Z_modalities: dict[str, torch.Tensor] | list[torch.Tensor] | tuple[torch.Tensor, ...]) -> torch.Tensor:
        ...
        idx = _sample_indices(flat_modalities[0].size(0), self.max_cross_samples, flat_modalities[0].device)
    ...
    def forward(
        ...
        temporal = self.temporal_contrast(Z)
        spatial = self.spatial_contrast(Z)

        if Z_modalities is not None:
            cross = self.cross_modal_contrast(Z_modalities)
        elif Z_m1 is not None and Z_m2 is not None:
            cross = self.infonce(
                self.projection_head(rearrange(Z_m1, "b t d h w -> (b t h w) d")),
                self.projection_head(rearrange(Z_m2, "b t d h w -> (b t h w) d")),
                symmetric=True,
            )
        else:
        ...
        refined = self.projection_head(Z[:, -1].mean(dim=(2, 3)).reshape(Z.size(0), -1))
        losses = {
            "temporal": temporal,
            "spatial": spatial,
            "cross": cross,
