...
Implements:
    - L_pred: wildfire risk regression loss (MSE / SmoothL1)
    - L_contrast: contrastive regularization loss
...
# -----------------------------------------------------------
# 1. Prediction Loss (Regression)
# -----------------------------------------------------------
class PredictionLoss(nn.Module):
    """
    Wildfire prediction loss for continuous risk regression.
    """
    ...
    def forward(self, y_pred, y_true):
        ...
        if self.loss_type in ["smoothl1", "smooth_l1", "huber"]:
...
class FireTrendLoss(nn.Module):
    ...
    def __init__(self, w_pred=1.0, w_contrast=0.5, w_phys=0.2, loss_type="mse"):
        super().__init__()
        self.w_pred = w_pred
    ...
    def forward(self, y_pred, y_true, embeddings=None, L_contrast=None, wind_u=None, wind_v=None):
        ...
        # Prefer externally provided contrastive loss from model.
        if L_contrast is None:
        ...
        elif not isinstance(L_contrast, torch.Tensor):
            L_contrast = torch.tensor(float(L_contrast), device=y_pred.device)
...
if __name__ == "__main__":
    ...
    loss_fn = FireTrendLoss(loss_type="smoothl1")
    losses = loss_fn(y_pred, y_true, embeddings=(z1, z2), wind_u=wind_u, wind_v=wind_v)
